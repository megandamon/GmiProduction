...
echo "$SCP_PATH $gmiEnvFile $REMOTE_USER@$MACH:$WORK_DIR"
echo "ready..."
$SCP_PATH $gmiEnvFile $REMOTE_USER@$MACH:$WORK_DIR
if [ "$?" != "0" ]; then
...
echo "$SCP_PATH $NED_WORKING_DIR/.cshrc.gmi-$MACH $REMOTE_USER@$MACH:"
$SCP_PATH $NED_WORKING_DIR/.cshrc.gmi-$MACH $REMOTE_USER@$MACH:
...
echo "$SCP_PATH $NED_WORKING_DIR/.login.gmi-$MACH $REMOTE_USER@$MACH:"
$SCP_PATH $NED_WORKING_DIR/.login.gmi-$MACH $REMOTE_USER@$MACH:
...
echo "$SSH_PATH $REMOTE_USER@$MACH chmod 755 .*.gmi* $WORK_DIR/.loadGMI*"
$SSH_PATH $REMOTE_USER@$MACH chmod 755 .*.gmi* $WORK_DIR/.loadGMI* 
#if [ "$?" != "0" ]; then
